...
if (process.argv.length < 2) {
  console.error("Usage: node getPicFV.js");
  process.exit();
}
...
if (cluster.isMaster) {
  var allList = [];
  var workers = {};
  var _content = fs.readFileSync("./update_list.txt", "utf-8");
...
*/  
  ...
  var generateCP = function () {
    ...
    tempWorker.on('message', function (msg) {
      ...
      if (msg.rt >= 3) {
        ...
        var _link = msg.link;
        allList[_indexL1]["list"][_indexL2].pic = msg.picList;
  ...
  function getPicList(_linkObj) {
    ...
    //http.get({url: _linkObj.link}, function(error, body) {
    //request({url: _linkObj.link, headers: {'accept-encoding': 'gzip,deflate'}}, function ( error, response, body ) {
      if (!error) {
        ...
        if (0) {
          zlib.gunzip(body.buffer, function(error, buffer) {
            if (!error) {
